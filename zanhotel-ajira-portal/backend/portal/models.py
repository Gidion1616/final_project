...
# Database models are the permanent data structure used by Django/SQLite.
class User(AbstractUser):
 """Login account plus Job Seeker documents and shared contact fields."""
...
class Hotel(models.Model):
 """Hotel business profile; approved controls Ministry access approval."""
...
class Job(models.Model):
 """A vacancy owned by one approved hotel."""
...
class Application(models.Model):
 """Connects one applicant to one job and stores the employer response."""
 STATUS=[('pending','Pending'),('accepted','Accepted'),('rejected','Rejected')];job=models.ForeignKey(Job,on_delete=models.CASCADE,related_name='applications');applicant=models.ForeignKey(User,on_delete=models.CASCADE,related_name='applications');applicant_note=models.TextField(blank=True);status=models.CharField(max_length=20,choices=STATUS,default='pending');feedback=models.TextField(blank=True);created_at=models.DateTimeField(auto_now_add=True);updated_at=models.DateTimeField(auto_now=True)
...
class ApiToken(models.Model):
 """Persistent random token sent by React to authenticate API requests."""
...
class SiteSetting(models.Model):
 """Administrator-controlled portal name, maintenance state, and job window."""
